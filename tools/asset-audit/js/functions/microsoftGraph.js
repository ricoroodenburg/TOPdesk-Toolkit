...
export async function getIntuneDevices({ graphToken, serialNumbers, extraFilter = '' }) {
...
export async function fetchUserBatch(graphToken, userIdsBatch) {
    ...
    try {
        ...
        const requests = userIdsBatch.map((userId, index) => ({
            id: `${index}`,
            method: 'GET',
            url: `/users/${userId}?$select=userPrincipalName,onPremisesSamAccountName,onPremisesSecurityIdentifier,id`,
        }));
